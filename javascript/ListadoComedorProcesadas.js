...
function MostrarInforme_green_spot(){
	...
	RowID = 0,
	numero_empleado_green_spot = 0;
	sede_green_spot = $("#sede_green_spot").val() != '' ? $("#sede_green_spot").val() : 'todos';
	$('.filter-option-inner-inner').text(sede_green_spot);
	if (Fecha == "") {
		Swal.fire( 
			"El campo fecha no puede ir vació",
			'',
	...
	let formData = new FormData(document.getElementById("form_comedor_conciliados"));
  	formData.append("dato", "valor");
	formData.append("param", 14);
	formData.append("sede_green_spot", sede_green_spot);
	$.ajax({
		url: "../../utileria.php",
		type: "post",
		data: formData,
